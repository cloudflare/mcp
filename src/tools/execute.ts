...
${list}`
  ...
  const count = props.accountCount !== undefined ? ` (${props.accountCount} accounts)` : ''
  return `

Account selection:
This token has access to multiple Cloudflare accounts${count}. ${ACCOUNT_DISCOVERY_GUIDANCE} Paginate as needed, or filter by exact name with GET /accounts?name=<exact account name>.`
}

function accountIdParamDescription(): string {
  return 'Cloudflare account ID for account-scoped API calls. Optional for account-independent calls.'
}
