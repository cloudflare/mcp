...
import type { AuthorizationToken } from '../../src/auth/cloudflare-auth'

vi.mock('../../src/auth/cloudflare-auth', async (importOriginal) => {
  const original = await importOriginal<typeof import('../../src/auth/cloudflare-auth')>()
  return {
    ...original,
...
async function mockRefreshAuthToken() {
  const { refreshAuthToken } = await import('../../src/auth/cloudflare-auth')
